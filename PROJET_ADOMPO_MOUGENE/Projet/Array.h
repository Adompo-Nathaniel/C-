...
namespace Array {
	...
	namespace Geometry {
		...
		template <class T> void Resize(T &array, float scaling) {
			if (array.size() == 0) return;
			typename T::type  center = Center(array);
			for (size_t i = 0; i < array.size(); i++) {
				array[i].scale(center, scaling);
				array[i].translate(center * scaling);
			}
		}
		...
		template <class T> void FillLosange(T &array, const typename T::type &center, float radius1, float radius2) {
			if (array.size() == 0) return;
			float angle = 2.f * Geometry::pi_f / float(array.size());
			for (size_t i = 0; i < array.size(); i++) {
				float r = i % 2 == 0 ? radius1 : radius2;
				array[i].polar(r, float(i)*angle, center);
			}
		}

		template <class T> void FillStar(T& array, const typename T::type& center, float radius) {
			if (array.size() == 0) return;
			float angle = Geometry::pi_f / 5.f;
			for (size_t i = 0; i < 10; i++) {
				float r = (i % 2 == 0) ? radius : (radius / 2.f);
				float theta = angle * float(i) - Geometry::pi_f/2.f; //- Geometry::pi_f/2.f pour la placer vers le haut
				array[i].polar(r, theta, center);
			}
		}

		template <class T> void FillParallelogramme(T& array, const typename T::type& center, float s1, float s2, float a) {
			if (array.size() == 0) return;
			float angle1 = a * Geometry::pi_f / 180.0f;
			float angle2 = Geometry::pi_f - angle1;
			array[0].polar(s1, -angle1 / 2.f, center);
			array[1].polar(s2, Geometry::pi_f - angle2 / 2.f, array[0]);
			array[2].polar(s1, -angle1 / 2.f + Geometry::pi_f, array[1]);
			array[3].polar(s2, Geometry::pi_f - angle2 / 2.f + Geometry::pi_f, array[2]);
		}

		template <class T> 
		void FillSun(T& array, const typename T::type& center, float radius) {
			if (array.size() == 0) return;
			float angle = Geometry::pi_f / 18.f;
			for (size_t i = 0; i < 36; i++) {
				float r = (i % 2 == 0) ? radius : (radius / 3.f);
				float theta = angle * float(i) - Geometry::pi_f/2.f;
				array[i].polar(r, theta, center);
			}
		...
		template <class T> 
		void FillSnowFlake(T& array, const typename T::type& center, float radius) {
			if (array.size() == 0) return;
			float angle = Geometry::pi_f / 6.f;
